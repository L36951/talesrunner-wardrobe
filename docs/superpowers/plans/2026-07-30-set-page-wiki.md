...
**⚠️ 呢個量度方法本身有陷阱，實作途中已經踩到一次。**

`、` 係普通字元，會計入「文字」；`<br>` 係標籤，會被剷走。所以當能力值嘅分隔符
由 `、` 改成 `<br>`（為咗令 26 條能力值嘅格睇得到），純文字長度會**跌**，
但一個字都冇少。純長度唔係中立指標。

**用以下兩個指標取代純長度：**

1. **內容字元**（剷走標籤之後，再剷走所有空白同 `、`）—— 分隔符唔算內容。
   實測：舊版 min 337 / median 944 / max 2908 → 新版 min 482 / median 1093 / max 3105，
   **0 版變少**。

2. **資料保存**（更重要）—— 唔理散文點改寫，直接問每個**裝備名、每條能力值、
   每個套裝效果、每個角色名**喺新版係咪仲搵得返。實測 1,978 版全部 **0 缺失**。

散文本身可以改寫。舊版嗰句可見 `summary`（`…查看每件裝備能力值、可穿著角色，並一鍵試身。`）
喺新版移咗去 meta／JSON-LD，可見位置換成資訊量更高嘅導語 —— 呢個係設計，唔係流失。
**淨係第 2 項出現非零先算退步**，嗰陣要停手報告。
